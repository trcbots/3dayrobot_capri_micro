...
class RCParser {
    ...
    private:
        ...
        const char* CLASS_NAME      = "RCParser";
...
// Initialise the RCParser
// pass true for debug to get Serial replies
RCParser::RCParser(uint8_t debug) {
    this->debug = debug;
}
...
void RCParser::parseExternalData(String data) {
...
uint16_t RCParser::getExpectedSteeringPosition() {
    return steering_position;
}

//uint16_t RCParser::getExpectedBrakePosition() {
 // return brake_position;
//}

uint16_t RCParser::getExpectedAcceleratorPosition() {
    return accelerator_position;
}

uint16_t  RCParser::getExpectedGearPosition() {
    return gear_position;
}

uint8_t  RCParser::getExpectedAutonomyStatus() {
    return start_status;
}

uint8_t  RCParser::getExpectedIgnitionStatus() {
...
void RCParser::loop(uint8_t rate) {
